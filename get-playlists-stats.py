...
import pandas as pd
from tools import spotipy_auth, read_ids_from_file, get_user_playlists, get_playlist_tracks, convert_ms_to_interval
...
def main():
    ...
    single_artists = []
    artists_durations = {}
    stats_playlists_names = ', '.join([f'"{playlist["name"]}"' for playlist in stats_playlists])
    ...
    with pd.option_context('display.max_rows', None, 'display.max_columns', None):
        ...
        cumulative_durations = { artist: [artists_durations[artist], convert_ms_to_interval(artists_durations[artist])] for artist in artists_durations }
        cumulative_durations = pd.DataFrame.from_dict(cumulative_durations, orient = 'index')
        if int(args.maxstats) >= len(cumulative_durations.index) / 2:
            cumulative_durations = cumulative_durations.sort_values(0, axis = 0, ascending = False).drop(0, axis = 1)
            print(f'\nCumulative durations:')
            print(cumulative_durations.to_string())
        else:
            max_stats = min(int(args.maxstats), len(cumulative_durations.index))
            longest = cumulative_durations.sort_values(0, axis = 0, ascending = False).drop(0, axis = 1)[:max_stats]
            shortest = cumulative_durations.sort_values(0, axis = 0, ascending = True).drop(0, axis = 1)[:max_stats]
        ...
        if int(args.maxstats) >= len(tracks_data.index) / 2:
            durations = tracks_data.sort_values('Duration-ms', axis = 0, ascending = False).drop('Duration-ms', axis = 1).drop('Album', axis = 1)
            print(f'\nSongs sorted by duration:\n{durations.to_string(index = False)}')
        else:
            max_stats = min(int(args.maxstats), len(tracks_data.index))
            longest = tracks_data.sort_values('Duration-ms', axis = 0, ascending = False).drop('Duration-ms', axis = 1).drop('Album', axis = 1)[:max_stats]
            shortest = tracks_data.sort_values('Duration-ms', axis = 0, ascending = True).drop('Duration-ms', axis = 1).drop('Album', axis = 1)[:max_stats]
            print('\nLongest songs:')
