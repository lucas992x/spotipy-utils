...
from tools import spotipy_auth, get_user_playlists, get_playlist_tracks, sort_tracks
...
def main():
    ...
    if args.file:
        ...
        input_file = os.path.join(script_dir, 'spotify-playlists-merge.json')
    ...
    for task in merge_tasks:
        ...
        if len(dest_tracks_ids_remove) > 0:
            print(f'Removing {len(dest_tracks_ids_remove)} exceeding track(s)')
        ...
        if len(source_tracks_uris_add) > 0:
            print(f'Adding {len(source_tracks_uris_add)} missing track(s)')
        ...
        if sorting:
            sort_tracks(dest_playlist_id, sp, sorting)
