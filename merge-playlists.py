...
def main():
    ...
    for task in merge_tasks:
        ...
        for item in dest_items:
            ...
            if item_id not in source_tracks_ids or lists_share_items(excluded_artists_ids, item_artists_ids):  # fmt: skip
                dest_tracks_ids_remove.append(item_id)
        if len(dest_tracks_ids_remove) > 0:
            print(f'Removing {len(dest_tracks_ids_remove)} exceeding track(s) from "{dest_playlist_name}"')  # fmt: skip
            sp.playlist_remove_all_occurrences_of_items(dest_playlist_id, dest_tracks_ids_remove)  # fmt: skip
        else:
        ...
        for item in source_items:
            ...
            item_artists_ids = [artist["id"] for artist in item["track"]["artists"]]
            if (
                item_id not in dest_tracks_ids
                and not item["track"]["uri"] in source_tracks_uris_add
                and not lists_share_items(excluded_artists_ids, item_artists_ids)
            ):
                source_tracks_uris_add.append(item["track"]["uri"])
        if len(source_tracks_uris_add) > 0:
            print(f'Adding {len(source_tracks_uris_add)} missing track(s) to "{dest_playlist_name}"')  # fmt: skip
            tracks_limit = 100
            for source_tracks_uris_add_chunk in split_list_in_chunks(source_tracks_uris_add, tracks_limit):  # fmt: skip
